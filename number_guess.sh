...
then
  ...
  GAMES_PLAYED=$($PSQL "SELECT COUNT(*) FROM games INNER JOIN USING users (user_id) WHERE username = '$USERNAME'")  
  BEST_GAME=$($PSQL "SELECT MIN(guesses) FROM games INNER JOIN USING users (user_id) WHERE username = '$USERNAME'")
